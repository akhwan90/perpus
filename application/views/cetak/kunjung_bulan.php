	...
	<table style="width: 50%; border-collapse: collapse" border="1" class="table table-condensed">
		...
		<?php 
		$c_jen_hi 	= $this->db->query("SELECT SUM(IF( jenis =  'Mahasiswa', 1, 0 ) ) AS mahasiswa, SUM( IF( jenis =  'Dosen', 1, 0 ) ) AS dosen FROM t_pengunjung  WHERE MID(tgl, 6, 2) = '$bulan' ")->result();
		
		foreach($c_jen_hi as $cjh) {
		?>
		<tr>
			<td style="text-align: center">1</td>
			<td style="text-align: center"><?=_null($cjh->mahasiswa)?></td>
			<td style="text-align: center"><?=_null($cjh->dosen)?></td>
			<td style="text-align: center"><?=_null(($cjh->mahasiswa + $cjh->dosen))?></td>
		</tr>	
		<?php
		}
		?>
