...
<button class="btn btn-primary" type="button" onclick="window.open('<?php echo base_URL(); ?>apps/r_lokasi/add/', '_self')">Data Baru</button>
<div class="pull-right">
<form action="<?=base_URL()?>apps/r_lokasi/cari" method="post">
...
<br><br>
	...
	<table width="100%"  class="table table-condensed">
		...
		<tr>
			...
			<a href="<?=base_URL()?>apps/r_lokasi/edt/<?=$b->id?>"><span class="icon-pencil">&nbsp;&nbsp;</span></a>  
			<a href="<?=base_URL()?>apps/r_lokasi/del/<?=$b->id?>" onclick="return confirm('Anda YAKIN menghapus data \n <?=$b->nama?>..?');"><span class="icon-remove">&nbsp;&nbsp;</span></a>
			</td>
		</tr>	
		<?php 
			}
